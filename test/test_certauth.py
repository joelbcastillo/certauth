...
def test_file_create_wildcard_host_cert_force_overwrite(ca):
    ret = main(
        [TEST_CA_ROOT, "-d", TEST_CA_DIR, "--hostname", "example.com", "-w", "-f"]
    )
    assert ret == 0
    certfile = os.path.join(TEST_CA_DIR, "example.com.pem")
...
def test_in_mem_parent_wildcard_cert():
    ...
    cert, key = ca.load_cert(
        "test.example.proxy", wildcard=True, wildcard_use_parent=True
    )
...
def test_custom_not_before_not_after():
    ...
    assert (
        abs(
            (
                time.mktime(actual_not_before.utctimetuple())
                - time.mktime(expected_not_before.utctimetuple())
            )
        )
        < 10
    ...
    assert (
        abs(
            (
                time.mktime(actual_not_after.utctimetuple())
                - time.mktime(expected_not_after.utctimetuple())
            )
        )
        < 10
    )
